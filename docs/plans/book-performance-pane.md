...
5. **Concentration** — **in progress.** Top-5 share of book value, sector split, cash %. Derived from the positions and account already on the page (no new polling or per-render fetches). Sector uses a static map of the desk universe plus an Other bucket. Guest / SIM uses the seeded book and never calls Alpaca. Read-only — `j`/`k` skip it.
